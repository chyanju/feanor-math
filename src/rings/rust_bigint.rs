...
/// assert_eq!(
///     "1267650600228230527396610048000",
///     format!("{}", ZZ.format(&a))
/// );
/// ```
/// and not
...
impl<A: Allocator + Clone> OrderedRing for RustBigintRingBase<A> {
    fn cmp(&self, lhs: &Self::Element, rhs: &Self::Element) -> Ordering {
        match (lhs.0, rhs.0) {
            ...
            (..) if self.is_zero(lhs) && self.is_zero(rhs) => Equal,
            (true, false) => Less,
            (false, true) => Greater,
        }
    }
...
impl<'de> Deserialize<'de> for RustBigintRingBase<Global> {
    ...
    where
        D: Deserializer<'de>,
    {
        DeserializeSeedNewtypeStruct::new("IntegerRing(RustBigInt)", PhantomData::<()>)
            .deserialize(deserializer)
            .map(|_| RustBigintRing::RING.into())
    }
}
...
fn test_print_power_2() {
    ...
    assert_eq!(
        ...
        format!("{}", RustBigintRing::RING.format(&x))
    );
}
...
fn test_to_i128() {
    ...
    assert_eq!(
        i64::MAX as i128 + 1,
        iso.map(RustBigint(false, vec![i64::MAX as u64 + 1]))
    );
...
fn bench_div_300_bits(bencher: &mut test::Bencher) {
    ...
    let z = RustBigintRing::RING
        .get_ring()
        .parse("48682207850683149082203680872586784064678018", 10)
        .unwrap();
    bencher.iter(|| {
...
fn test_canonical_iso_static_int() {
    // for the hom test, we have to be able to multiply elements in `StaticRing::<i128>::RING`, so
    // we cannot test `i128::MAX` or `i128::MIN`
    crate::ring::generic_tests::test_hom_axioms(
        StaticRing::<i128>::RING,
        ZZ,
        [0, 1, -1, -100, 100, i64::MAX as i128, i64::MIN as i128]
            .iter()
            .copied(),
    );
