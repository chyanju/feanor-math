...
/// Contains an implementation [`multivariate_impl::MultivariatePolyRingImpl`] of a multivariate
/// polynomial ring.
...
pub trait MultivariatePolyRing: RingExtension {
    ...
    /// Returns the **L**eading **T**erm of `f`, i.e. the term whose monomial is largest w.r.t. the
    /// given order.
    ...
    /// monomials smaller than `lt_than`.
    ...
    ///     poly_ring.evaluate(
    ///         &f,
    ///         [5, 8].clone_ring_els(StaticRing::<i64>::RING),
    ...
    /// which can allow a faster implementation sometimes. In particular, this only replaces a
    /// single indeterminate, and does not change the ring.
    ...
    Self::Type: MultivariatePolyRing,
{
    ...
    ///             poly_ring.create_monomial([0, 0, 0]),
    ///         ),
    ///         (
    ///             base_ring.int_hom().map(2),
    ///             poly_ring.create_monomial([0, 1, 1]),
    ///         ),
    ///         (base_ring.one(), poly_ring.create_monomial([2, 0, 0])),
    ///     ]
    ///     .into_iter(),
    /// );
    /// let f_version2 = poly_ring
    ///     .with_wrapped_indeterminates_dyn(|[x, y, z]| [3 + 2 * y * z + x.pow_ref(2)])
    ///     .into_iter()
    ///     .next()
    ///     .unwrap();
    /// let [f_version3] =
    ///     poly_ring.with_wrapped_indeterminates(|[x, y, z]| [3 + 2 * y * z + x.pow_ref(2)]);
...
/// orders.
///
/// To be precise, a total order on monomials is a monomial order, if for all `u, v, w` with `u <=
/// v` also `uw <= vw`.
...
/// ]
/// .into_iter()
/// .map(|m| poly_ring.create_monomial(m))
/// .collect::<Vec<_>>();
/// for i in 1..6 {
///     assert!(
///         DegRevLex.compare(
///             &poly_ring,
///             &monomials_descending[i - 1],
///             &monomials_descending[i]
///         ) == Ordering::Greater
///     );
/// }
/// ```
